  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-30 right-1/4 text-4xl animate-bounce" style={{ animationDelay: '0.7s' }}>🌈</div>
      </div>

      <Card className="w-full max-w-lg mx-4 bg-white shadow-2xl border-4 border-sky-400 relative z-10">
        <CardHeader className="text-center pb-2">
          {/* 吉祥物图片 */}
          <div className="flex justify-center mb-4">
            <img src="/images/mascot.svg" alt="吉祥物" className="w-24 h-24" />
          </div>
          <div className="text-6xl mb-2">{rank.emoji}</div>
          <CardTitle className="text-3xl font-bold text-gray-800">
            游戏结束
          </CardTitle>
          <p className={`text-lg font-bold ${rank.color} mt-2`}>
            {rank.message}
          </p>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          {/* 等级评定 */}
          <div className="text-center py-4 bg-gradient-to-r from-sky-100 to-sky-50 rounded-xl border-2 border-sky-200">
            <div className="text-sm text-gray-500 mb-2">最终等级</div>
            <div className={`text-8xl font-bold ${rank.color} drop-shadow-lg star-twinkle`}>
              {rank.grade}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {/* 分数 */}
            <div className="p-3 bg-sky-50 rounded-xl text-center border-2 border-sky-200">
            ...
            {/* WPM */}
            <div className="p-3 bg-green-50 rounded-xl text-center border-2 border-green-200">
              <div className="text-xs text-gray-500 mb-1">⚡ 速度</div>
              <div className="text-2xl font-bold text-green-600">{wpm}</div>
            </div>

            {/* 准确率 */}
            <div className="p-3 bg-purple-50 rounded-xl text-center border-2 border-purple-200">
            ...
            {/* 最大连击 */}
            <div className="p-3 bg-orange-50 rounded-xl text-center border-2 border-orange-200">
              <div className="text-xs text-gray-500 mb-1">🔥 连击</div>
              <div className="text-2xl font-bold text-orange-600">{maxCombo}</div>
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-4">
            ...
            <Button
              onClick={onRestart}
              className="flex-1 h-12 text-lg bg-sky-500 hover:bg-sky-600"
            >
              🔄 再来一局
            </Button>
          </div>
